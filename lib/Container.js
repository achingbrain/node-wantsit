  ...
  var readDirectory = function (directory) {
    fs.readdirSync(directory).forEach(function (file) {
      ...
      if (file.substr(0, 1).toUpperCase() === file.substr(0, 1)) {
        ...
        this._logger.debug('Container', 'Registering', fileName, 'from', fullPath)
        this.register(fileName, require(fullPath))
      }
    }.bind(this))
  }.bind(this)
  readDirectory(path)
...
Container.prototype._getByType = function (type) {
  ...
  this.emit('error', new Error('No component with type ' + type + ' has been registered'))
}

Container.prototype._getDependency = function (name, optional) {
  if (!this._store[name] && !optional) {
    this.emit('error', new Error('No component with name ' + name + ' has been registered'))
